...
export default function DocsPage() {
  ...
  const handleViewDocument = async (doc: Document) => {
    try {
      ...
      // Get the signed URL for the original document
      const { data: signedData, error: signedError } = await supabase.storage
        .from("documents")
      ...
      // If the document is signed, get the signature images
      let signatureUrls = {};
      if (
        doc.signature_status === "completed" &&
        doc.signature_data?.signatures
      ) {
        const signaturePromises = doc.signature_data.signatures.map(
          async (sig: any) => {
            const { data: signatureUrl, error: signatureError } =
              await supabase.storage
                .from("signatures")
                .createSignedUrl(sig.path, 60);

            if (signatureError) {
              console.error(
                `Error getting signature URL for ${sig.path}:`,
                signatureError
              );
              return null;
            }
        ...
        signatureUrls = signatures.reduce((acc, sig) => {
          if (sig) {
            acc[sig.fieldId] = {
              url: sig.url,
              position: sig.position,
  ...
  const revertSignatureStatus = async (doc: Document) => {
    try {
      setLoading(true);
      console.log("Reverting signature status for document:", doc.id);
      ...
      if (docError) {
        console.error("Document update error:", docError.message);
        throw new Error(`Failed to update document: ${docError.message}`);
      }

      if (!updatedDoc) {
        throw new Error("No document data returned after update");
      }

      // Update any associated signature requests
      const { data: updatedRequests, error: sigReqError } = await supabase
      ...
      if (sigReqError) {
        console.error("Signature request update error:", sigReqError.message);
        throw new Error(
          `Failed to update signature requests: ${sigReqError.message}`
        );
      }

      console.log("Successfully reset signature status:", {
        document: updatedDoc,
        signatureRequests: updatedRequests,
      });

      toast.success("Document signature status reset successfully");
      router.refresh();
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "An unexpected error occurred";
      console.error("Error resetting signature status:", errorMessage);
      toast.error(`Failed to reset signature status: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
                  ...
                  {doc.signature_status === "completed" ? (
                    <div className="mt-2 space-y-1">
                      ...
                      <p className="text-sm text-green-600">
                        Signed on:{" "}
                        {doc.last_signed_at
                          ? new Date(doc.last_signed_at).toLocaleDateString()
                          : "Unknown"}
                      </p>
                    </div>
                  ) : (
                      ...
                      <>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              confirm(
                                "Are you sure you want to revert the signature status? This will allow the document to be signed again."
                              )
                            ) {
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setViewerDialog((prev) => ({ ...prev, isOpen: false }))
        }
      >
        <DialogContent className="max-w-7xl h-[90vh]">
          ...
          <div className="flex-1 overflow-auto">
            {viewerDialog.url && (
              <PDFViewer
                url={viewerDialog.url}
                signatures={viewerDialog.signatures}
              />
            )}
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() =>
                setViewerDialog((prev) => ({ ...prev, isOpen: false }))
              }
            >
              Close
            </Button>
